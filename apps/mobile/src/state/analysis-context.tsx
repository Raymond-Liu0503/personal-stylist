...
const Context=createContext<{state:AnalysisState;setPrompt:(s:string)=>void;prepare:(p:Photo,c?:Crop)=>Promise<PreparationOutcome>;timer:0|3|10;setTimer:(n:0|3|10)=>void;submit:()=>Promise<void>;cancel:()=>Promise<void>}>({state:initialAnalysis,setPrompt:()=>{},prepare:async()=>({status:'cancelled'}),timer:0,setTimer:()=>{},submit:async()=>{},cancel:async()=>{}});
...
export function AnalysisProvider({children}:{children:React.ReactNode}){
 ...
 const cancel=async()=>{active.current?.abort();dispatch({type:'cancel'});await work.current?.catch(()=>{});await cleanupImages();dispatch({type:'reset'});};
 ...
 const submit=async()=>{const current=stateRef.current;if(active.current||!current.prepared||!['ready','failed'].includes(current.status))return;
  ...
  const task=(async()=>{try{const result=await analyze(current.prepared!.uri,{schemaVersion:1,prompt:current.prompt},requestId,controller.signal);if(!controller.signal.aborted)dispatch({type:'result',requestId,result});await cleanupImages().catch(()=>{});}catch(e){if(!controller.signal.aborted)dispatch({type:'failed',message:e instanceof Error?e.message:'Analysis failed.',controller});}finally{if(active.current===controller)active.current=null;void refresh().catch(()=>{});}})();work.current=task;await task;};
 return <Context.Provider value={{state,setPrompt:prompt=>dispatch({type:'prompt',prompt}),prepare,submit,cancel,timer,setTimer}}>{children}</Context.Provider>;
}
